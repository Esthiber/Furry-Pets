...
namespace PawfectMatch.Services.Veterinaria
{
    public class CarritoService
    {
        ...
        public async Task<bool> ActualizarCantidad(int carritoId, int nuevaCantidad)
        {
            await using var contexto = await _dbFactory.CreateDbContextAsync();
            await using var transaction = await contexto.Database.BeginTransactionAsync();
            ...
            try
            {
                var item = await contexto.Carrito.Include(c => c.Producto).FirstOrDefaultAsync(c => c.CarritoId == carritoId);
